...
TEST_CASE("SimplePath") {
  ...
  GIVEN("a point") {
    ...
    GIVEN("a path with two points") {
      ...
      GIVEN("a path copied with the assignment operator") {
        ...
        THEN("the size of the path should be two") { REQUIRE(ipath().size() == 2); }
    ...
    GIVEN("a path with three points") {
      ...
      GIVEN("a path distance generated from the path") {
        ...
        THEN("the size of the path should be one") { REQUIRE(ipath().size() == 1); }
      ...
      GIVEN("a path generated with two interpolations") {
        ...
        THEN("the size of the path should be five") { REQUIRE(ipath2().size() == 5); }
      ...
      GIVEN("a path generated with five interpolations") {
        ...
        THEN("the size of the path should be eleven") { REQUIRE(ipath2().size() == 11); }
  ...
  GIVEN("A path with 4 segments and 10 interpolations") {
    ...
    THEN("the size of the path should be 31") { REQUIRE(path().size() == 31); }
  }
  ...
  GIVEN("a path with three segments") {
    ...
    WHEN("the path is smoothed") {
      ...
      THEN("the starting point should not move") { CHECK(*path().at(0) == (Vector {0_in, 0_in})); }

      THEN("the ending point should not move") { CHECK(*path().at(2) == (Vector {5_in, 0_in})); }

      THEN("the middle point x should not move") { CHECK(path().at(1)->x == 2.5_in); }

      THEN("the middle point y should move") { CHECK(path().at(1)->y != 4_in); }
    }
  }
  ...
  GIVEN("a generated and interpolated path") {
    ...
    THEN("the size of the path should be 21") { REQUIRE(path().size() == 21); }
  ...
  GIVEN("a path with three segments generated by distance") {
    ...
    THEN("the path should have 13 points") { CHECK(path().size() == 13); }
  ...
  GIVEN("a path with angles") {
    ...
    THEN("the path should have 21 points") { CHECK(path().size() == 21); }
