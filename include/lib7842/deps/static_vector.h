#ifndef STD_EXPERIMENTAL_STATIC_VECTOR
#define STD_EXPERIMENTAL_STATIC_VECTOR
/// \file
///
...
#define FCV_REQUIRES_(...)                                                                         \
  int FCV_CONCEPT_PP_CAT(_concept_requires_,                                                       \
                         __LINE__) = 42,                                                           \
...
#define FCV_REQUIRES(...)                                                                          \
  template <int FCV_CONCEPT_PP_CAT(_concept_requires_, __LINE__) = 42,                             \
            typename ::std::enable_if<(FCV_CONCEPT_PP_CAT(_concept_requires_, __LINE__) == 43) ||  \
                                        (__VA_ARGS__),                                             \
                                      int>::type = 0> /**/
...
template <typename T> static constexpr bool Trivial = is_default_constructible_v<T>;
...
constexpr bool cmp(InputIterator1 first1, InputIterator1 last1, InputIterator2 first2,
...
public:
  ...
  template <typename... Args,
            FCV_REQUIRES_(Constructible<T, Args...>and Assignable<value_type&, T>)>
...
private:
  ...
  using aligned_storage_fixed_t =
    aligned_storage_t<sizeof(remove_const_t<T>), alignof(remove_const_t<T>)>;
  using data_t = conditional_t<!Const<T>, aligned_storage_fixed_t, const aligned_storage_fixed_t>;
...
template <typename T, size_t Capacity>
struct static_vector : private fcv_detail::storage::_t<T, Capacity> {
private:
...
public:
  ...
  constexpr reference at(size_type pos) {
    if (FCV_UNLIKELY(pos >= size())) { throw out_of_range("static_vector::at"); }
    return fcv_detail::index(*this, pos);
  }
  ...
  constexpr iterator emplace(const_iterator position, Args&&... args) noexcept(
    ...
    FCV_EXPECT(!full() && "tried emplace on full static_vector!");
  ...
  constexpr iterator
    ...
    FCV_EXPECT(!full() && "tried insert on full static_vector!");
    assert_iterator_in_range(position);
  ...
  template <class InputIt,
            FCV_REQUIRES_(fcv_detail::InputIterator<InputIt>and fcv_detail::Constructible<
  ...
  constexpr void swap(static_vector& other) noexcept(is_nothrow_swappable_v<T>) {
  ...
  constexpr void resize(size_type sz, T const& value) noexcept(is_nothrow_copy_constructible_v<T>) {
    if (sz == size()) { return; }
    if (sz > size()) {
      FCV_EXPECT(sz <= capacity() && "static_vector cannot be resized to "
...
private:
  ...
  constexpr void emplace_n(size_type n) noexcept(
    ...
    FCV_EXPECT(n <= capacity() && "static_vector cannot be "
...
public:
  ...
  constexpr static_vector(InputIt first, InputIt last) {
    if constexpr (fcv_detail::RandomAccessIterator<InputIt>) {
      ...
      FCV_EXPECT(static_cast<size_type>(last - first) <= capacity() &&
  ...
  constexpr static_vector(initializer_list<U> il) noexcept(noexcept(base_t(move(il)))) :
...
template <typename T, size_t Capacity>
constexpr bool operator==(static_vector<T, Capacity> const& a,
                          static_vector<T, Capacity> const& b) noexcept {
  return a.size() == b.size() and
         fcv_detail::cmp(a.begin(), a.end(), b.begin(), b.end(), equal_to<> {});
}
...
#endif // STD_EXPERIMENTAL_STATIC_VECTOR
